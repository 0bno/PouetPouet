...
describe('PIVOT_MODULES manifest', () => {
  it('has at least one module', () => {
    expect(PIVOT_MODULES.length).toBeGreaterThan(0)
  })

  it('all module ids are unique kebab-case strings', () => {
    const ids = PIVOT_MODULES.map((m) => m.id)
  ...
  it('nav hrefs are unique across all modules', () => {
    const hrefs = PIVOT_MODULES.flatMap((m) => m.nav.map((l) => l.href))
  ...
  it('every event in listensTo is emitted by exactly one other module', () => {
    const allEmitted = new Set(PIVOT_MODULES.flatMap((m) => m.emits))
    for (const m of PIVOT_MODULES) {
